...
class RegistrationController extends GetxController {
  ...
  Future<void> initializeRegistration() async {
    try {
      ...
      ServexUtils.dLog('Error initializing registration: $e');
      Get.snackbar(
        'Error',
  ...
  void onSubcategorySelected(int subcategoryId, bool isSelected) {
    ...
    if (state != null) {
      ...
      ServexUtils.dLog('Subcategory $subcategoryId selected: $isSelected');
    }
  }
  ...
  Future<void> submitAddressDetails() async {
    ...
    try {
      ...
      ServexUtils.dPrint('vendorId: $userId');
      ServexUtils.dPrint('address: "${addressController.text.trim()}"');
      ServexUtils.dPrint('state: "${stateController.text.trim()}"');
      ServexUtils.dPrint('city: "${cityController.text.trim()}"');
      ServexUtils.dPrint('pincode: "${pinCodeController.text.trim()}"');
      ServexUtils.dPrint(
  ...
  Future<void> submitWorkDetails() async {
    if (!workFormKey.currentState!.validate()) {
      ServexUtils.dLog('Work form validation failed');
      return;
    }
    ...
    try {
      ...
      final charges = subcategoryCharges
          .map(
            (e) => {
              ...
              'service_charge': e.serviceCharge,
            },
          )
          .toList();
      ...
      ServexUtils.dLog('Authorization token: $token');
      ...
      if (response.statusCode == 200) {
        ...
        ServexUtils.dLog('Work details submission failed: $errorMessage');
        Get.snackbar(
          'Error',
          errorMessage,
          backgroundColor: AppColors.errorColor,
      ...
      ServexUtils.dLog('Unexpected error in submitWorkDetails: $e');
      Get.snackbar(
        'Error',
  ...
  Future<void> submitDocuments() async {
    if (!documentFormKey.currentState!.validate()) {
      ServexUtils.dLog('Document form validation failed');
      return;
    }
    ...
    try {
      ...
      ServexUtils.dLog('Submitting documents for vendor_id=$userId');
      ...
      if (response.statusCode == 200) {
        ...
        ServexUtils.dLog('Document upload failed: $errorMessage');
        Get.snackbar(
          'Error',
          errorMessage,
          backgroundColor: AppColors.errorColor,
      ...
      ServexUtils.dLog('DioException in submitDocuments: $errorMessage');
      Get.snackbar(
        'Error',
        errorMessage,
        backgroundColor: AppColors.errorColor,
      ...
      ServexUtils.dLog('Unexpected error in submitDocuments: $e');
      Get.snackbar(
        'Error',
