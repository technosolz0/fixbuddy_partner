...
class LocalStorage {
  ...
  Future<bool> getIsOnboarded() async {
    return bool.parse(
      await pref.read(key: isOnboardedKey, aOptions: _getAndroidOptions()) ??
          'false',
    );
  }
  ...
  Future<void> setVendorDetails(VendorModel vendor) async {
    await pref.write(
      key: vendorDetailsKey,
      value: jsonEncode(vendor.toJson()),
      aOptions: _getAndroidOptions(),
    );
  ...
  Future<void> setRegistrationStep(int step) async {
    try {
      String? registrationData = await pref.read(
  ...
  Future<Map> getMapDataFromLocal(String key) async {
    String data =
        await pref.read(key: key, aOptions: _getAndroidOptions()) ?? "{}";
